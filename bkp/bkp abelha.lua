...
on_rightclick = function(self, clicker,rot,jogador,pos)
		...
		if jogador == self.owner then
			...
			if ent.nectar >=2 then
				--minetest.chat_send_all("Voce pegou "..ent.nectar.." de meu")
				minetest.sound_play("abelha_1", {pos = pos, gain = 0.5, max_hear_distance = 3, loop = false })
				ent.nectar=ent.nectar -1  
				self.nametag=ent.nectar.." de POLEN"
				clicker:get_inventory():add_item("mine_apiario:mine_apiario", "mine_apiario:nectar")
			else
				--minetest.chat_send_all("nao tenho mel soficiente "..ent.nectar.."")
				minetest.sound_play("abelha_2", {pos = pos, gain = 0.5, max_hear_distance = 3, loop = false })
			end
		else
				minetest.sound_play("abelha_3", {pos = pos, gain = 0.5, max_hear_distance = 3, loop = false })
    end 
